...
public interface ExchangeRateRepository extends JpaRepository<ExchangeRateEntity, ExchangeRatePK>{
	...
	@Query("SELECT tc FROM ExchangeRateEntity tc WHERE ( tc.registdate BETWEEN :startAt AND :finalAt )"
			+ " AND ( :origen = '' OR tc.origen = :origen )"
			+ " AND ( :destin = '' OR tc.destin = :destin )")
	List<ExchangeRateEntity> findByLike(@Param("startAt") LocalDate startAt,
